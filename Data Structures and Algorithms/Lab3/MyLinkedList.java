...
public class MyLinkedList<E> implements ListInterface<E> {
    ...
    public E removeCurr(Node<E> curr) {
        ...
        if (size() == 1)
            return removeFirst();
        //if the linked list contains more than 1 item but the curr is actually the head node.
    ...
    public void addSortedList(E item) {
        ...
        if(head == null) addFirst(item);
            ...
            Node<E> tmp = head;
            while(tmp.getNext() != null){
                if (tmp.getNext().getData() instanceof Integer && item instanceof Integer){
                    int temp = (Integer)tmp.getNext().getData();
                    int t = (Integer) item;
                    if (temp >= t){
                        break;
                    }
